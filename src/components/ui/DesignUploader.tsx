        ...
        {[
          ...
          <button key={mode} type="button" onClick={() => setMode(mode)}
            ...
            style={value.mode === mode
              ? { background: 'rgba(244,130,31,0.08)', border: '1.5px solid #DC2626', color: 'var(--text-primary)' }
              : { background: 'var(--bg-secondary)', border: '1.5px solid var(--border)', color: 'var(--text-secondary)' }}>
            <div className="flex items-center gap-1.5">
              <Icon size={13} style={{ color: value.mode === mode ? '#DC2626' : 'var(--text-muted)' }} />
      ...
      {value.mode === 'support' && (
        <div className="space-y-3">
          <div className="flex items-start gap-2 p-3 rounded-lg"
            ...
            <Check size={13} className="text-[#DC2626] mt-0.5 flex-shrink-0" />
        ...
        <div className="mt-4 pt-4" style={{ borderTop: '1px solid var(--border)' }}>
          ...
          <label className="flex items-start gap-2.5 cursor-pointer group">
            <div
              ...
              style={{
                background: value.termsAccepted ? '#DC2626' : 'var(--bg-secondary)',
                border: value.termsAccepted ? '2px solid #DC2626' : '2px solid var(--border)',
              }}>
              {value.termsAccepted && <Check size={11} className="text-white" />}
            </div>
            <p className="text-[12px] leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
              <button type="button" onClick={() => setTermsOpen(true)}
                className="font-bold underline hover:text-[#DC2626] transition-colors"
