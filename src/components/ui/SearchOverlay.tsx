        ...
        <div className="overflow-y-auto flex-1 pb-4">
            ...
            <div className="space-y-0.5">
              {catResults.map(cat => (
                <Link key={cat.slug} href={`/katalog/${cat.slug}`} onClick={onClose}
                  ...
                  style={{ color: query ? '#E63946' : 'var(--text-secondary)' }}>
                ...
                {results.map(p => (
                  <Link key={p.slug} href={`/urun/${p.slug}`} onClick={onClose}
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[13px] font-bold truncate group-hover:text-[#E63946] transition-colors"
                        style={{ color: 'var(--text-primary)' }}>
                        {p.name}
                      </p>
                      {p.shortDesc && (
                      ...
                      <p className="text-[10px] mt-0.5" style={{ color: 'var(--text-muted)' }}>
                        Kategori <span style={{ color: '#E63946' }}>{p.categoryName}</span>
                      </p>
                    </div>
                  </Link>
                ))}
          ...
          {query.trim().length >= 2 && results.length === 0 && (
            <div className="px-4 mt-6 text-center py-6">
              ...
              <button onClick={() => handleSearch(query)}
                className="mt-3 text-[12px] font-bold text-[#E63946] hover:underline">
                Yine de ara →
              </button>
            </div>
          )}
