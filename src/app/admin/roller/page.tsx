                  ...
                  return (
                    <div key={cat}>
                      ...
                      <div className="grid grid-cols-3 gap-1.5">
                        {perms.map(p => (
                          <label key={p.id} className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer border transition-colors ${newRole.permissionIds.has(p.id) ? 'border-[#E63946] bg-orange-50 dark:bg-orange-500/10' : 'border-black/[0.06] dark:border-white/[0.06] hover:bg-gray-50 dark:hover:bg-white/[0.03]'}`}>
                            <div className={`w-4 h-4 rounded flex items-center justify-center flex-shrink-0 border transition-colors ${newRole.permissionIds.has(p.id) ? 'bg-[#E63946] border-[#E63946]' : 'border-gray-300 dark:border-gray-600'}`}>
                            ...
                            <div key={cat}>
                              ...
                              <div className="grid grid-cols-3 gap-1.5">
                                {perms.map(p => {
                                  ...
                                  return (
                                    <label key={p.id} className={`flex items-center gap-2 p-2 rounded-lg border transition-colors ${isEditing ? 'cursor-pointer' : 'cursor-default'} ${checked ? 'border-[#E63946] bg-orange-50 dark:bg-orange-500/10' : 'border-black/[0.06] dark:border-white/[0.06]'} ${isEditing && !checked ? 'hover:bg-gray-100 dark:hover:bg-white/[0.04]' : ''}`}>
                                      <div className={`w-4 h-4 rounded flex items-center justify-center flex-shrink-0 border transition-colors ${checked ? 'bg-[#E63946] border-[#E63946]' : 'border-gray-300 dark:border-gray-600'}`}>
                      ...
                      {isEditing && (
                        <div className="flex gap-2 justify-end">
                          ...
                          <button onClick={() => saveEdit(role.id)} disabled={saving}
                            className="text-[12px] font-medium bg-[#E63946] text-white px-3 py-1.5 rounded-lg hover:opacity-90 transition-opacity disabled:opacity-50">
