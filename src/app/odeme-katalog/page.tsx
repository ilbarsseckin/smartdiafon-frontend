        ...
        <div className="max-w-lg mx-auto px-4 sm:px-6 py-8">
            ...
            <div className="rounded-2xl p-4 mb-4"
              ...
              <div className="space-y-1.5 pt-3" style={{ borderTop: '1px solid var(--border)' }}>
                ...
                <div className="flex justify-between items-baseline pt-2" style={{ borderTop: '1px solid var(--border)' }}>
                  ...
                  <span className="text-[24px] font-black text-[#DC2626]">₺{fmtTL(displayTotal)}</span>
                </div>
              </div>
            </div>
          )}
                ...
                <>
                  {welcomeCoupon && (
                    <button onClick={() => applyCoupon(welcomeCoupon)} disabled={couponLoading}
                      ...
                      <Gift size={16} className="text-[#DC2626] flex-shrink-0" />
                      <div className="text-left flex-1 min-w-0">
                        <p className="text-[12px] font-bold text-[#DC2626]">Hoş geldin kuponunuz var!</p>
                        <p className="text-[10px]" style={{ color: 'var(--text-muted)' }}>
                          {welcomeCoupon} — uygulamak için tıkla
                        </p>
                      </div>
                      <span className="text-[10px] font-bold text-[#DC2626]">Uygula →</span>
                    </button>
                  )}
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => applyCoupon()} disabled={couponLoading || !couponInput.trim()}
                      className="px-5 py-2.5 text-[12px] font-bold rounded-xl bg-[#DC2626] text-white hover:opacity-90 transition-all disabled:opacity-40">
          ...
          {files.length > 0 && (
            <div className="rounded-2xl p-4 mb-4"
              style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
              <div className="flex items-center gap-2 mb-3">
                <Paperclip size={14} style={{ color: '#DC2626' }} />
          ...
          <div className="rounded-2xl p-5 space-y-4"
            ...
            <button onClick={handleSubmit} disabled={paymentLoading}
              ...
              style={{ background: 'linear-gradient(135deg, #DC2626, #b91c1c)', boxShadow: '0 6px 14px rgba(244,130,31,0.3)' }}>
              {paymentLoading
