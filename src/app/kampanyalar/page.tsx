    ...
    <>
      ...
      <main className="min-h-[60vh] max-w-7xl mx-auto px-4 sm:px-6 py-10">
        <div className="flex items-center gap-2.5 mb-2">
          <Megaphone size={26} style={{ color: '#E63946' }} />
        ...
        {loading ? (
          <div className="flex justify-center py-24">
            <Loader2 size={30} className="animate-spin" style={{ color: '#E63946' }} />
          </div>
        ) : items.length === 0 ? (
...
function CampaignCard({ c }: { c: Campaign }) {
  const card = (
    <div className="relative rounded-2xl overflow-hidden h-full group"
      ...
      <div className="relative h-[180px] overflow-hidden">
        ...
        {c.badgeText && (
          <span className="absolute top-3 left-3 text-[12px] font-bold px-2.5 py-1 rounded-lg text-white shadow"
            style={{ background: c.badgeColor || '#E63946' }}>
            {c.badgeText}
          </span>
        )}
      </div>
      <div className="p-5">
        {c.label && (
          <p className="text-[11px] font-bold uppercase tracking-wide mb-1" style={{ color: '#E63946' }}>
            {c.label}
          </p>
        )}
